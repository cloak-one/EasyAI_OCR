// ===== compile.ino — XIAO ESP32S3 Sense: Camera + Mic (PDM) + Speaker (I2S) =====
// ===== 按需摄像头 + WebSocket 语音交互 =====
...
#include <math.h>  // sqrt for RMS calculation
#include <WiFiClient.h>
using namespace websockets;

// ===== WiFi / Server =====
const char *WIFI_SSID = "LENOVOY70005178";
const char *WIFI_PASS = "2*2j57J3";
...
const int AUDIO_QUEUE_DEPTH = 6;               // 【降低】减少缓冲深度，提高实时性
const int MIC_NOISE_GATE_RMS = 15;             // 【新增】噪声门限，低于此值不发送
const unsigned long MIC_SEND_INTERVAL_MS = 25; // 【新增】发送节流间隔(ms)
...
volatile bool streaming_enabled = false; // 【新增】按需流控：需收到 CAMERA_STREAM:START 才发帧
...
void clear_frame_queue()
{
  ...
  while (xQueueReceive(qFrames, &drop, 0) == pdPASS)
  {
    if (drop)
...
bool parse_json_int_field(const String &json, const char *field, int &out)
{
  ...
  if (p >= json.length())
    return false;
  out = json.substring(p).toInt();
  return true;
}

bool parse_json_string_field(const String &json, const char *field, String &out)
{
  String key = String("\"") + field + "\":";
  int i = json.indexOf(key);
  if (i < 0)
...
bool framesize_from_name(String name, framesize_t &fs)
{
  name.trim();
  name.toUpperCase();
  if (name == "QQVGA")
...
bool framesize_from_dimensions(int width, int height, framesize_t &fs)
{
...
bool init_camera()
{
  ...
  if (s)
  {
    ...
    s->set_hmirror(s, 0); // 与人眼左右一致（1=开，0=关）
    s->set_vflip(s, 1);   // 若镜头倒装可改为 1

    // 先恢复通用自动控制基线，避免沿用 OV3660 的手动曝光经验值
    s->set_gain_ctrl(s, 1);
    ...
    if (pid == OV3660_PID)
    {
      ...
      Serial.println("[CAM] tuned for OV3660");
    }
    else if (pid == OV5640_PID)
    {
      // 5MP 级 sensor 在同样环境下更容易偏暗，给一点更亮的默认风格
      s->set_brightness(s, 1);
      s->set_contrast(s, 0);
      s->set_saturation(s, 0);
      Serial.println("[CAM] tuned for OV5640-class sensor");
    }
    else
    {
      // 其他 sensor 先走偏保守的自动曝光基线，后续按串口 PID 再细调
      s->set_brightness(s, 1);
      s->set_contrast(s, 0);
      s->set_saturation(s, 0);
      Serial.println("[CAM] tuned for generic auto-exposure baseline");
    }
  }
  return true;
}
...
void taskCamSend(void *)
{
  ...
  for (;;)
  {
    ...
    {
      if (fb && cam_ws_ready && streaming_enabled)
      {
    ...
    else
    {
      // 队列接收超时，检查是否长时间没有帧
      unsigned long now = millis();
      if (cam_ws_ready && streaming_enabled && last_sent_time > 0 && (now - last_sent_time) > 3000)
      {
...
void taskMicCapture(void *)
{
  ...
  for (;;)
  {
    if (run_audio_stream && aud_ws_ready)
    {
      ...
      int rms = (int)(sqrt((double)sum_sq / samples_per_chunk) + 0.5);
...
void taskMicUpload(void *)
{
  ...
  for (;;)
  {
    if (run_audio_stream && aud_ws_ready)
    {
      ...
      {
        // 【新增】发送节流：控制发送间隔，避免过快冲击 ASR
        unsigned long now = millis();
        if (last_send_time > 0 && (now - last_send_time) < MIC_SEND_INTERVAL_MS)
        {
          delay(MIC_SEND_INTERVAL_MS - (now - last_send_time));
        }
...
static bool readN_http_body(WiFiClient &cli, uint8_t *buf, size_t n, bool chunked, size_t &chunk_left, uint32_t timeout_ms = 3000)
{
  ...
  while (got < n)
  {
    ...
    else
    {
      ...
      {
        String szline;
        if (!read_line(cli, szline, timeout_ms))
          return false;
        int sc = szline.indexOf(';');
        if (sc >= 0)
          szline = szline.substring(0, sc);
        szline.trim();
...
{
  uint8_t hdr12[12];
  if (!readN_http_body(cli, hdr12, 12, chunked, chunk_left))
    return false;
  ...
  {
    uint8_t chdr[8];
    if (!readN_http_body(cli, chdr, 8, chunked, chunk_left))
      return false;
    ...
    if (memcmp(chdr, "fmt ", 4) == 0)
    {
      ...
      if (!readN_http_body(cli, fmtbuf, toread, chunked, chunk_left))
        return false;
      ...
      while (left)
      {
        ...
        if (!readN_http_body(cli, dump, d, chunked, chunk_left))
          return false;
        left -= d;
      }
...
{
  Serial.println("[AUDIO] http_wav task entered");
  WiFiClient cli;
    ...
    return [&](uint8_t *dst, size_t n, uint32_t timeout_ms) -> bool
    {
      ...
      while (filled < n)
      {
        ...
        if (is_chunked)
        {
          ...
          {
            char crlf[2];
            if (!readNRaw((uint8_t *)crlf, 2, 200))
              return false;
          }
        }
        else
        {
          if (!readNRaw(dst + filled, n - filled, timeout_ms))
            return false;
          filled = n;
        }
      }
  ...
  while (true)
  {
    ...
    while (http_play_running)
    {
      ...
      if (!readBody(inbuf, bytes20, BODY_TIMEOUT_MS))
      {
        break;
      }
      filled = bytes20;
      ...
      {
        if (!readBody(inbuf + filled, bytes20, 2))
        {
          break;
        }
        filled += bytes20;
  ...
  // unreachable: 常驻任务，依靠 http_play_running 控制工作/空闲状态
}
...
void startStreamWav()
{
  if (taskHttpPlayHandle)
  {
    ...
    Serial.println("[AUDIO] http_wav task resumed");
  }
  else
  {
    http_play_running = true;
    BaseType_t rc = xTaskCreatePinnedToCore(taskHttpPlay, "http_wav", 6144, nullptr, 2, &taskHttpPlayHandle, 0);
    if (rc == pdPASS && taskHttpPlayHandle)
    {
    ...
    else
    {
      ...
      Serial.printf("[AUDIO] http_wav task create failed: %ld\n", (long)rc);
    }
  }
}
void stopStreamWav()
{
  if (!taskHttpPlayHandle || !http_play_running)
    return;
  http_play_running = false;
  Serial.println("[AUDIO] http_wav task paused");
}
...
void setup()
{
  ...
  http_play_running = false;
  BaseType_t http_rc = xTaskCreatePinnedToCore(taskHttpPlay, "http_wav", 6144, nullptr, 2, &taskHttpPlayHandle, 0);
  if (http_rc == pdPASS && taskHttpPlayHandle)
    Serial.println("[AUDIO] http_wav task precreated");
  else
  {
    taskHttpPlayHandle = nullptr;
    Serial.printf("[AUDIO] http_wav task precreate failed: %ld\n", (long)http_rc);
  }
  ...
  wsCam.onEvent([](WebsocketsEvent ev, String)
    ...
    if (ev == WebsocketsEvent::ConnectionOpened)  {
      cam_ws_ready = true;
      streaming_enabled = false;  // 【新增】默认不推流，等待 CAMERA_STREAM:START
      Serial.println("[WS-CAM] open (streaming OFF)");
      // 重置统计
      frame_sent_count = 0;
  ...
  wsCam.onMessage([](WebsocketsMessage msg)
    ...
    if (msg.isText()){
      ...
      // 【新增】按需流控命令
      if (cmd == "CAMERA_STREAM:START") {
        streaming_enabled = true;
        Serial.println("[CAM] stream ON");
      }
      else if (cmd == "CAMERA_STREAM:STOP") {
        streaming_enabled = false;
        Serial.println("[CAM] stream OFF");
      }
      // 【新增】CAMERA_PROFILE:JSON — 解析并应用 profile
      else if (cmd.startsWith("CAMERA_PROFILE:")) {
        String json = cmd.substring(strlen("CAMERA_PROFILE:"));
        int q = -1, f = -1, width = -1, height = -1;
        String fs_name;
        parse_json_int_field(json, "jpeg_quality", q);
        parse_json_int_field(json, "fps", f);
        parse_json_int_field(json, "width", width);
        parse_json_int_field(json, "height", height);
        framesize_t requested_fs = g_frame_size;
        bool has_framesize = false;
        if (parse_json_string_field(json, "framesize", fs_name)) {
          has_framesize = framesize_from_name(fs_name, requested_fs);
        }
        if (!has_framesize && width > 0 && height > 0) {
          has_framesize = framesize_from_dimensions(width, height, requested_fs);
        }
        if (has_framesize && requested_fs != g_frame_size) {
          snapshot_in_progress = true;
          clear_frame_queue();
          if (apply_framesize(requested_fs)) {
            Serial.printf("[CAM] profile framesize=%s\n", framesize_name(requested_fs));
          } else {
            Serial.printf("[CAM] profile framesize failed: %s\n", framesize_name(requested_fs));
          }
        ...
        if (q > 0) {
          ...
          Serial.printf("[CAM] profile quality=%d\n", q);
        }
        if (f > 0) {
          g_target_fps = constrain(f, 1, 60);
          Serial.printf("[CAM] profile fps=%d\n", g_target_fps);
        }
      }
        ...
        if (!framesize_from_name(v, fs)) fs = g_frame_size;
        snapshot_in_progress = true;
        clear_frame_queue();
