...
def _extract_sentence(event_obj: Any) -> Tuple[Optional[str], Optional[bool]]:
    ...
    for obj in cands:
        ...
        if isinstance(sent, str):
            end_val = obj.get("sentence_end", obj.get("is_sentence_end", obj.get("final")))
            is_end = bool(end_val) if end_val is not None else None
            return sent, is_end
        if isinstance(sent, dict):
            text = sent.get("text")
            if isinstance(text, list):
                text = "".join(str(x) for x in text if x is not None)
            is_end = sent.get("sentence_end", sent.get("is_sentence_end", sent.get("final")))
    ...
    for obj in cands:
        if "text" in obj and isinstance(obj.get("text"), str):
            end_val = obj.get("sentence_end", obj.get("is_sentence_end", obj.get("final")))
            is_end = bool(end_val) if end_val is not None else None
            return obj.get("text"), is_end
    return None, None

# ====== 仅热词触发的“全清零复位”配置 ======
INTERRUPT_KEYWORDS = set(
    os.getenv("INTERRUPT_KEYWORDS", "停下,别说了,停止,停一下,算了,不说了,别说了,打住,停下来").split(",")
)
...
def _compact_text(s: str) -> str:
    """Normalize text for robust hotword matching by removing spaces and punctuation."""
    t = _normalize_cn(s)
    if not t:
        return ""
...
class ASRCallback:
    ...
    def _handle(self, event: Any):
        ...
        if not self._hot_interrupted and self._has_hotword(text):
            self._hot_interrupted = (is_end is not True)
        ...
        # 热词已命中过当前句：吞掉本句剩余事件，避免 final 再次进入 LLM
